 ...
 *
 * This covers running out of stock, which is the error that matters most to a customer
 * of this product — and it also covers confirming a reservation that was already
 * confirmed. The server reports both as FAILED_PRECONDITION, so the SDK cannot tell them
 * apart without reading the message text, which would be brittle. Known limitation: the
 * fix is server-side, returning RESOURCE_EXHAUSTED for missing stock.
 */
